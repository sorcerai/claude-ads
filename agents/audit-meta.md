...
4. Confirm account, date window, timezone, currency, objective, and available
   inputs from immutable warehouse snapshots with provenance and verified freshness.
   Analytical agents cannot call Meta account-read tools directly. Mark missing
...
Severity comes from the ControlDefinition. Workers MUST NOT emit or invent `severity`. `score_contribution` is not a v2 field (it is not merely worker-omitted; deterministic scoring calculates scores downstream and rejects findings with legacy fields). Workers MUST omit `score_contribution` and legacy fields. Do not emit legacy `result` or `evidence_refs`. Validate the complete result against the v2 finding schema before return.
...
Warehouse snapshots bind immutable `fetched_at` and `extracted_at` with a 15-minute platform freshness SLA and 28-day finalization semantics (`data_finalization: provisional | final`). Never infer freshness from `measurement_context.as_of` (which records data coverage window end) or `EvidenceRecord.observed_at`. Stale snapshots fail closed before scoring or reporting.
