...
def _repository(tmp_path: Path) -> Path:
    ...
    for evidence in sorted(
        (source_root / "control-plane/dependency-evidence").glob("*.json")
    ):
...
def test_audit_checks_frontmatter_and_sensitive_content(tmp_path: Path) -> None:
    ...
    _write(
        root, "notes/fable_audit_prompt.txt", "captured prompt must not be tracked.\n"
    )
    _git(root, "add", ".")
    errors = audit_repository(root)
...
@pytest.mark.parametrize(
    "case", ["top-field", "bool-size", "float-size", "archive-field", "product"]
)
def test_release_manifest_schema_and_types_fail_closed(
...
def test_release_verifier_requires_trusted_commit_and_exact_checksum_set(
    tmp_path: Path,
) -> None:
    root = _repository(tmp_path)
    artifacts = build_release(root, tmp_path / "dist")
...
def test_external_runtime_manifest_is_exact_and_archived(tmp_path: Path) -> None:
    ...
    assert {item["id"] for item in document["dependencies"]} == {
...
def test_sbom_fails_closed_on_missing_version_or_license(
    tmp_path: Path, field: str
) -> None:
    root = _repository(tmp_path)
    _rewrite_inventory(
        root, lambda inventory: inventory["component_catalog"][0].__setitem__(field, "")
    )
    with pytest.raises(
        ReleaseError, match="invalid name/version|lacks a reviewed license"
    ):
...
def test_sbom_rejects_duplicate_components(tmp_path: Path) -> None:
    ...
    _rewrite_inventory(
        root,
        lambda inventory: inventory["component_catalog"].append(
            copy.deepcopy(inventory["component_catalog"][0])
        ),
    )
...
def test_sbom_rejects_direct_requirement_coverage_or_constraint_mismatch(
    tmp_path: Path,
) -> None:
    root = _repository(tmp_path)
...
def test_lock_target_hash_and_marker_parity_fail_closed(tmp_path: Path) -> None:
    ...
    _rewrite_inventory(
        root,
        lambda inventory: inventory["targets"][0]["components"][0][
            "artifact"
        ].__setitem__("sha256", "0" * 64),
    )
    with pytest.raises(
        ReleaseError,
        match="target component evidence mismatch|lock target artifact mismatch",
    ):
        build_sbom(root, "claude-ads", "2.0.0")

    parsed = release._parse_hash_lock(
        RELEASE_SCRIPT.parents[1] / "requirements-dev.lock"
    )
...
def test_notice_inventory_has_no_dangling_references_and_records_bundled_terms() -> (
    ...
    assert {
        "pyphen-selected-wheel-documents",
        "reportlab-selected-wheel-documents",
        "matplotlib-selected-wheel-documents",
    } <= notices
    artifacts = {
    ...
    covered = {
        ...
        for digest in document["artifact_sha256s"]
    } | {
        digest
        for notice in inventory["bundled_notices"]
        for digest in notice["documentless_artifact_sha256s"]
    }
    assert covered == artifacts and len(artifacts) == 119
    webencodings = next(
        item
        for item in inventory["bundled_notices"]
        if item["component"] == "webencodings"
    )
    assert (
        not webencodings["documents"]
        and len(webencodings["documentless_artifact_sha256s"]) == 1
    )
    urllib3 = next(
        item for item in inventory["component_catalog"] if item["name"] == "urllib3"
    )
...
def test_inventory_provenance_and_graph_tampering_fail_closed(
    tmp_path: Path, case: str
) -> None:
    root = _repository(tmp_path)
    ...
    def mutate(inventory):
        if case == "header":
            ...
            inventory["targets"][0]["components"][0]["artifact"]["filename"] = (
                "wrong.whl"
            )
        elif case == "dangling-edge":
            inventory["dependency_edges"].append(
                {
                    "profile": "runtime",
                    "from": "requests",
                    "to": "missing",
...
def test_standalone_verify_rejects_self_consistent_sbom_semantic_tamper(
    tmp_path: Path,
) -> None:
    root = _repository(tmp_path)
    artifacts = build_release(root, tmp_path / "dist")
    ...
    checksum_lines = [
        ...
        if line.endswith("  sbom.cdx.json")
        else line
...
def test_wheel_tag_policy_rejects_cross_target_and_boundary_swaps(
    filename: str, target_id: str
) -> None:
...
def test_inventory_rejects_bundled_notice_forgery(tmp_path: Path, case: str) -> None:
    ...
    def forge(inventory):
        if case == "remove":
            ...
            next(
                item
                for item in inventory["component_catalog"]
                if item["name"] == "fonttools"
            )["bundled_notice_ids"] = []
        elif case == "text":
            ...
            inventory["bundled_notices"][0]["documents"][0]["artifact_sha256s"].append(
                "0" * 64
            )
        else:
            next(
...
def test_inventory_rejects_arbitrary_license_and_header_policy(tmp_path: Path) -> None:
    ...
    _rewrite_inventory(
        root,
        lambda inventory: inventory["component_catalog"][0].__setitem__(
            "license_expression", "Definitely-A-License"
        ),
    )
    ...
    for field, value in (
        ("managed_lock_python_range", ">=3.11"),
        ("source_date_epoch", 0),
        ("policy", "looks fine"),
    ):
        ...
        _rewrite_inventory(
            candidate,
            lambda inventory, field=field, value=value: inventory[
                "resolution"
...
def test_normalized_target_evidence_is_honest_about_foreign_source_and_native_ci_requirement() -> (
    None
):
    root = RELEASE_SCRIPT.parents[1]
    linux = json.loads(
        (root / "control-plane/dependency-evidence/runtime-linux-cp311.json").read_text(
            encoding="utf-8"
        )
    )
    windows = json.loads(
        (
            root / "control-plane/dependency-evidence/development-windows-cp311.json"
        ).read_text(encoding="utf-8")
    )
    assert (
        linux["evidence_class"]
    ...
    assert (
        windows["normalization_notes"]
        and "colorama" in windows["normalization_notes"][0]
    )


def test_standalone_verify_rejects_self_consistent_archive_inventory_tamper(
    tmp_path: Path,
) -> None:
    root = _repository(tmp_path)
    artifacts = build_release(root, tmp_path / "dist")
    ...
    inventory_member = (
        f"{archive_root}/control-plane/manifests/dependency-inventory.json"
    )
    ...
    inventory[
        "component_catalog"
    ].reverse()  # Semantically equivalent, but not the reviewed bytes.
    ...
    record = next(
        ...
        if item["path"] == "control-plane/manifests/dependency-inventory.json"
    )
    record.update(
        size=len(inventory_bytes), sha256=hashlib.sha256(inventory_bytes).hexdigest()
    )
    ...
    with pytest.raises(
        ReleaseError, match="trusted Git commit|independently reviewed document"
    ):
        verify_release(tmp_path / "dist", _commit(root), root)
...
def test_claude_command_contract_distinguishes_plugin_namespace() -> None:
    root = RELEASE_SCRIPT.parents[1]
    product = json.loads(
        (root / "control-plane/manifests/product-manifest.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    boundaries = (root / "control-plane/PRODUCT_BOUNDARIES.md").read_text(
        encoding="utf-8"
    )
...
def test_release_grounding_gate_fails_closed_on_demoted_load_bearing_claim(
    tmp_path: Path,
) -> None:
    root = RELEASE_SCRIPT.parents[1]
...
def test_release_gate_fails_closed_without_external_model_review_and_ci_evidence() -> (
    None
):
    root = RELEASE_SCRIPT.parents[1]
...
def test_release_gate_forwards_external_model_trust_inputs(
